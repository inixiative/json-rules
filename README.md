...
| `ruleSourceValues(lens, rule)` | The values a rule names at each source the lens declares, keyed like `projectByPath` (`path` + `field`, with the source's `mapName` / `model`); `dynamic: true` when a leaf took its value from `path` / `bind` / `variable`. Nested and dotted relation spellings are one path; no-value operators contribute nothing. The reverse question for a reference registry ("which rows does this rule name") — join `model` + `values`. |
